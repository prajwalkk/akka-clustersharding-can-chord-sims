...
//  final case class updateOthers(i :Int) extends Command
////,replyTo:ActorRef[updateOthersResponse]
//
//
//
//  // Responses to Self
...
//  case class updateOthersResponse() extends Command
//
...
//  case class NodeVar(nodeName:String,nodeId:Int,Fingervar:List[FingerTableEntity],sucessorref:ActorRef[Command],predecessorref:ActorRef[Command])
//  var FingerTablevar :List[FingerTableEntity] = List.empty
...
//    this.FingerTablevar = fingerTable
...
//  }
//
////  def update_others(context: ActorContext[Command],actorRef: ActorRef[Command]): Unit ={
////    logger.info("In update others")
////    logger.info(s"Update others in ${context.self.path.name}")
////    implicit val timeout: Timeout = Timeout(10.seconds)
////    implicit val scheduler: Scheduler = context.system.scheduler
////
////    for(i <- 0 to 8-1){
////      logger.info((hashNodeRef(actorRef) - Math.pow(2,i).toInt).toString)
//////      val future :Future[updateOthersResponse] = actorRef ? updateOthers
//////      val resp = Await.result(future,timeout.duration)
////      val p = findPredecessor(context,(hashNodeRef(actorRef) - Math.pow(2,i).toInt),actorRef)
////      Thread.sleep(10000)
////      logger.info(s"Update others ${p.path.toString}")
////    }
////
////  }
//  def update_finger_table(actorRef: ActorRef[Command],i:Int): Unit ={
//
//  }
//
...
//      val no =initFingerTable(currNodeID, oldFingerTable, nodeRef, context)
//      logger.info("End init")
//     selfAddress ! updateOthers(0)
//      //update_others(context,selfAddress)
//      Thread.sleep(10000)
//      logger.info("End init")
//      no
//      // TODO update others
...
//          implicit val timeout: Timeout = Timeout(10.seconds)
//          implicit val scheduler: Scheduler = context.system.scheduler
//          val id = (hashNodeRef(context.self) - Math.pow(2,i).toInt)
//          var nDash = context.self
//          var nDashNode = hashNodeRef(context.self)
//          var nDashSuccessor = hashFingerTableEntity(this.FingerTablevar.head)
//          var nDashFingerTable = this.FingerTablevar
//
////          context.log.info("-----------------")
////          context.log.info(s"${context.self}")
////          context.log.info(s"${this.FingerTablevar}")
////          context.log.info("-----------------")
//          //val p = findPredecessor(context,(hashNodeRef(context.self) - Math.pow(2,i).toInt),context.self)
//          //context.log.info(s"update others${p.path}")
...
//            logger.info("Returned from closest finger")
...
//          }
//          context.log.info("In updateothers case")
//          Behaviors.same
//        }
//
//        case Join(nodeRef) =>
...
//            context.log.debug(s"Updated fingerTable:  ${Helper.getIdentifier(i.node.get.path.name)}")}
//          //context.log.info(s"Finger Table updated for ${context.self.path}")
//          this.FingerTablevar = newFingerTable
...
//    logger.info("In predecessor")
//    logger.debug(s"Finding Predecessor")
//    logger.info(s"${context.self.path} ==== ${existingNode}")
...
//      implicit val timeout: Timeout = Timeout(10.seconds)
//      implicit val scheduler: Scheduler = context.system.scheduler
//      logger.info(s"Finding finger table for $nDash")
//      val future: Future[FingerTableResponse] = nodeDash ? GetFingerTable
//      val resp = Await.result(future, timeout.duration)
//      logger.info("Found finger table")
//      resp.fingerTable
//    }
//
//
...
//    var nDashSuccessor = Helper.getIdentifier(nDashFingerTable.head.node.get.path.name)
//    var nDashNode = Helper.getIdentifier(nDash.path.name)
//    while (!isIdentifierInIntervalLeftExclusive(id, Array(nDashNode, nDashSuccessor))) {
//
//      nDash = closestPrecedingFinger(id, nDashFingerTable, nDash)
//      logger.info("Returned from closest finger")
