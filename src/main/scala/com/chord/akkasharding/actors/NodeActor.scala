...
import com.chord.akkasharding.actors.NodeGroup.{NodeSnapshot, ReplyDataSnapshot, ReplySnapshot}
import com.chord.akkasharding.simulation.Simulation.select_random_node
import com.chord.akkasharding.utils.{Helper, SystemConstants}
import com.chord.akkasharding.utils.Helper.rangeValidator
...
object NodeActor extends LazyLogging {
  ...
  def apply(nodeName: String, sharding:ClusterSharding, typeKey:EntityTypeKey[Command]): Behavior[Command] = {
    // Each node has a:
    // Node ID
    // Node Name
    // Node Successor
    ...
    Behaviors.setup { context =>
      val nodeId = Helper.getIdentifier((sharding.entityRefFor(typeKey,nodeName)).toString)
      ...
      new NodeActor(nodeName, context,sharding,typeKey).nodeBehaviors(nodePropertiesWhenCreated)
    }
  ...
  final case class Join(nDash: EntityRef[NodeActor.Command]) extends Command
  ...
  final case class SetPredecessor(nodeRef: EntityRef[NodeActor.Command]) extends Command
  ...
  case class GetLookupResponse(maybeObject: Option[LookupObject]) extends Command
  ...
  case class NodeSetup(nodeName: String,
                       ...
                       nodeSuccessor: Option[EntityRef[NodeActor.Command]],
                       nodePredecessor: Option[EntityRef[NodeActor.Command]],
    ...
    override def toString: String = {
      s"node :$nodeName \n nodeRef :${nodeRef} " +
        s"\n nodeSuccessor : ${nodeSuccessor.get} " +
        s"\n nodePredecessor :${nodePredecessor.get}," +
        s"\n nodeFingerTable: \n${
...
class  NodeActor private(name: String,
                        context: ActorContext[NodeActor.Command],sharding: ClusterSharding,typeKey: EntityTypeKey[NodeActor.Command]) {
  ...
  private def nodeBehaviors(nodeProperties: NodeSetup): Behavior[Command] = {
    ...
    Behaviors.receiveMessage {
      case Join(nDash) =>
        ...
        //added
        //val n = context.self
        val n = sharding.entityRefFor(typeKey,context.self.path.name)
        context.log.info(s"Ref_n : ${n.toString}, Ref n_dash: ${nDash.toString}")
        val nFingerTable = nodeProperties.nodeFingerTable
        if (n.toString.equals(nDash.toString)) {
          //context.log.info(s"[Join] First node $name joining: n: ${n.path.name} with nDash : ${nDash.path.name}")
          context.log.info(s"[Join] First node $name joining: n: ${n} with nDash : ${nDash}")
      ...
      case FindSuccessor(id, replyTo) =>
        context.log.debug(s"[FindSuccessor] FindSuccessor id $id context ${context.self.path.name} ${replyTo} ${replyTo}")
        ...
        if (nDashRef.toString.equals(getEntity(context.self.path.name).toString)) {
          context.log.debug(s"[FindSuccessor] if Finding Predecessor for ${nDash.nodeName}")
          val nDashSuccessor = nodeProperties.copy()
          replyTo ! ReplyWithSuccessor(nDashSuccessor)
        } else {
          context.log.debug(s"[FindSuccessor] else Finding Predecessor for ${nDash.nodeName}")
        ...
        context.log.debug(s"[FindSuccessor] Result for $id ${nDash.nodeSuccessor.get}")
        Behaviors.same
      ...
      case ClosestPrecedingFinger(id, replyTo) =>
        context.log.debug(s"[${context.self.path.name}] [${context.self.path}][${context.self}]Executing closest preceding finger. replyTo: ${replyTo}  replyTo: ${replyTo}  replyTo: ${replyTo}")
        val n = closest_preceding_finger(id, nodeProperties)
        replyTo ! ReplyWithClosestPrecedingFinger(n)
        context.log.debug(s"[${context.self.path.name}] Executed n = ${n.toString}. Reply done: ${replyTo} ")
        Behaviors.same
      //Not Required
      ...
      case SetPredecessor(nodeRef) =>
        context.log.debug(s"[SetPredecessor] ${context.self.path.name} Setting my predecessor to ${nodeRef}")
      ...
      case UpdateOthers =>
        ...
        for (i <- 0 until SystemConstants.M) {
          ...
          context.log.debug(s"[UpdateOthers] Calling update finger table p: ${p}  UpdateFingerTable(${n.nodeName}, $i)  ")
      ...
      case UpdateFingerTable(sNodeSetup: NodeSetup, i: Int) =>
        ...
        val newNodeProperties = if (rangeValidator(leftInclude = true, n.nodeID, hashNodeRef(nFinger(i).node.get), rightInclude = false, s) && (!sNodeSetup.nodeRef.toString.equals(getEntity(context.self.path.name).toString))) {
          ...
          context.log.debug(s"[UpdateFingerTablep]${p} ${sNodeSetup.nodeName}")
      ...
      case GetNodeProperties(replyTo) =>
        context.log.info(s"[GetNodeProperties]Reply to ${replyTo}")
        context.log.info(s"[GetNodeProperties]Context ${context.self}")
        val nodePropertiesCopy = nodeProperties.copy()
        context.log.info(s"[GetNodeProperties] Contents  ${nodePropertiesCopy.toString}")
      ...
      case FindNode(requestObject, replyTo) =>
        ...
        context.log.debug(s"[FindNode] random node selected ${node} finding successor to add data with key: $id")
        ...
        context.log.debug(s"[FindNode] Found successor ${resp.nodeSuccessor.get} to store data key: $id")
      ...
      case SearchDataNode(key, replyTo) =>
        ...
        context.log.debug(s"[SearchDataNode] random node selected ${node} finding successor to locate data with key: $id")
        // context.log.info(s"Random node found ${node.path.name}")
        val future: Future[ReplyWithSuccessor] = node.ask(ref => FindSuccessor(id, ref))
        val resp = Await.result(future, timeout.duration).nSuccessor
        context.log.debug(s"[SearchDataNode] Found successor ${resp.nodeSuccessor.get} locating data with key: $id")
  ...
  private def find_predecessor(id: Int, nodeProperties: NodeSetup): NodeSetup = {
    ...
    while (!rangeValidator(leftInclude = false, nDash.nodeID, hashNodeRef(nDash.nodeSuccessor.get), rightInclude = true, id)) {
      context.log.debug(s"[find_predecessor]  inside rangeValidator ${nDash.nodeRef} right  ${n.nodeRef}")
      if (nDash.nodeRef.toString.equals(n.nodeRef.toString)) {
        ...
        context.log.debug(s"[find_predecessor] ${context.self.path.name} in Find Predecessor ${nDash.nodeRef} ")
      }
    }
  ...
  private def closest_preceding_finger(id: Int,
    ...
    for (i <- SystemConstants.M - 1 to 0 by -1) {
      ...
      if (rangeValidator(leftInclude = false, nId, id, rightInclude = false, hashFingerTableEntity(nFingerTable(i)))) {
        ...
        context.log.debug(s"[closest_preceding_finger] ${fingerNodeIRef}")
        context.log.debug(s"[closest_preceding_finger] ${getEntity(context.self.path.name)}")
        if (fingerNodeIRef.toString.equals(getEntity(context.self.path.name).toString)) {
          context.log.debug(s"[closest_preceding_finger] if condition")
          ...
          //TODO problem here as well find_predecessor-> closest_preceding_finger
          context.log.debug(s"[closest_preceding_finger] else condition ${fingerNodeIRef}")
          implicit val timeout: Timeout = Timeout(10.seconds)
          implicit val scheduler: Scheduler = context.system.scheduler
          context.log.debug(s"[closest_preceding_finger] context ${context.self}")
          val future: Future[ReplyWithNodeProperties] = fingerNodeIRef.ask{ref =>
            context.log.debug(s"[closest_preceding_finger] else condition reply to ${ref.path.name}")
            GetNodeProperties(ref)}
          val nDash = Await.result(future, timeout.duration).nodeSetup
          context.log.debug(s"[closest_preceding_finger] else condition after wait")
          return nDash
        }
  ...
  private def init_finger_table(nDash: EntityRef[Command],
                                nodeProperties: NodeSetup): NodeSetup = {

    // line1 of algo to find successor
    // and update curr node successor
    val n = getEntity(context.self.path.name)
    context.log.debug(s"[init_finger_table] init finger table n ${n} nDash ${nDash}")
    implicit val timeout: Timeout = Timeout(10.seconds)
    implicit val scheduler: Scheduler = context.system.scheduler
    val nFingerTable = nodeProperties.nodeFingerTable
    ...
    context.log.info(s"[init_finger_table] Setting predecessor for ${nodePropsOfNewSuccessor.nodePredecessor.get} as ${n}  ")
  ...
  private def hashFingerTableEntity(f: FingerTableEntity): Int = {
    Helper.getIdentifier(f.node.get.toString)
  }

  // returns ID of the ActorRef

  private def hashNodeRef(f: EntityRef[Command]): Int = {
    Helper.getIdentifier(f.toString)
  }
